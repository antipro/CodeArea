 ...
 * @author antipro
 */
public abstract class CodeInputControlBehavior<T extends CodeInputControl> extends BehaviorBase<T> {
    ...
    public CodeInputControlBehavior(T c) {
        super(c);
        ...
        InputMap.KeyMapping cancelEditMapping;
        InputMap.KeyMapping fireMapping;
        InputMap.KeyMapping consumeMostPressedEventsMapping;
        ...
        addDefaultMapping(inputMap,
                ...
                // Traversal Bindings
                new InputMap.KeyMapping(new KeyBinding(TAB), FocusTraversalInputMap::traverseNext),
                new InputMap.KeyMapping(new KeyBinding(TAB).shift(), FocusTraversalInputMap::traversePrevious),
                new InputMap.KeyMapping(new KeyBinding(TAB).ctrl(), FocusTraversalInputMap::traverseNext),
                new InputMap.KeyMapping(new KeyBinding(TAB).ctrl().shift(), FocusTraversalInputMap::traversePrevious),

                // The following keys are forwarded to the parent container
                cancelEditMapping = new InputMap.KeyMapping(ESCAPE, this::cancelEdit),
                ...
                keyMapping(new KeyBinding(null, KEY_TYPED)
                                .alt(KeyBinding.OptionalBoolean.ANY)
                                .shift(KeyBinding.OptionalBoolean.ANY)
                                .ctrl(KeyBinding.OptionalBoolean.ANY)
                                .meta(KeyBinding.OptionalBoolean.ANY),
                        this::defaultKeyTyped),

                // However, we want to consume other key press / release events too, for
                // things that would have been handled by the InputCharacter normally
                consumeMostPressedEventsMapping =
                        keyMapping(new KeyBinding(null, KEY_PRESSED).shift(KeyBinding.OptionalBoolean.ANY),
                                e -> { if (!e.getCode().isFunctionKey()) e.consume(); }),

                // VK
                new InputMap.KeyMapping(new KeyBinding(DIGIT9).ctrl().shift(), e -> {
    ...
    protected void addKeyPadMappings(InputMap<T> map) {
        ...
        for (Object o : map.getMappings()) {
            if (o instanceof InputMap.KeyMapping) {
                InputMap.KeyMapping mapping = (InputMap.KeyMapping)o;
                ...
                if (kb.getCode() != null) {
                    ...
                    if (newCode != null) {
                        KeyBinding newkb = new KeyBinding(newCode).shift(kb.getShift())
                                ...
                                .alt(kb.getAlt())
                                .meta(kb.getMeta());
                        tmpMap.getMappings().add(new InputMap.KeyMapping(newkb, mapping.getEventHandler()));
                    }
                }
            }
        }
     ...
     */
    protected InputMap.KeyMapping keyMapping(final KeyCode keyCode, final EventHandler<KeyEvent> eventHandler) {
        return keyMapping(new KeyBinding(keyCode), eventHandler);
    }

    protected InputMap.KeyMapping keyMapping(KeyBinding keyBinding, final EventHandler<KeyEvent> eventHandler) {
    ...
    private void defaultKeyTyped(KeyEvent event) {
        final CodeInputControl textInput = getNode();
    ...
    private void nextCharacterVisually(boolean moveRight) {
        if (isMixed()) {
            CodeInputControlSkin<?> skin = (CodeInputControlSkin<?>)textInputControl.getSkin();
            skin.moveCaret(CodeInputControlSkin.TextUnit.CHARACTER, moveRight ? CodeInputControlSkin.Direction.RIGHT : CodeInputControlSkin.Direction.LEFT, false);
    ...
    protected void deletePreviousWord() {
        setEditing(true);
        CodeInputControl codeInputControl = getNode();
        int end = codeInputControl.getCaretPosition();

        if (end > 0) {
            codeInputControl.previousWord();
            int start = codeInputControl.getCaretPosition();
    ...
    protected void deleteNextWord() {
        ...
        if (start < codeInputControl.getLength()) {
            nextWord();
            int end = codeInputControl.getCaretPosition();
            replaceText(start, end, "");
        }
    ...
    public void deleteSelection() {
        setEditing(true);
        CodeInputControl codeInputControl = getNode();
        IndexRange selection = codeInputControl.getSelection();
    ...
    public void selectNextWord() {
        CodeInputControl codeInputControl = getNode();
        if (isMac() || isLinux()) {
            codeInputControl.selectEndOfNextWord();
        } else {
            codeInputControl.selectNextWord();
        }
    }
    ...
    protected void selectWord() {
        final CodeInputControl codeInputControl = getNode();
        codeInputControl.previousWord();
        if (isWindows()) {
            codeInputControl.selectNextWord();
        } else {
    ...
    protected void nextWord() {
        CodeInputControl codeInputControl = getNode();
        if (isMac() || isLinux()) {
            codeInputControl.endOfNextWord();
        } else {
            codeInputControl.nextWord();
        }
    }
    ...
    protected void selectEndExtend() {
        CodeInputControl codeInputControl = getNode();
        codeInputControl.extendSelection(codeInputControl.getLength());
    }
    ...
    protected void populateContextMenu() {
        CodeInputControl codeInputControl = getNode();
        boolean editable = codeInputControl.isEditable();
        boolean hasText = (codeInputControl.getLength() > 0);
        boolean hasSelection = (codeInputControl.getSelection().getLength() > 0);
        boolean allSelected = (codeInputControl.getSelection().getLength() == codeInputControl.getLength());
        boolean maskText = false; // (maskText("A") != "A");
