...
public abstract class CodeInputControl extends Control {
    /**
     ...
     */
    protected CodeInputControl(final Content content) {
            ...
            font = new StyleableObjectProperty<Font>(Font.getDefault()) {
                ...
                protected void invalidated() {
                    ...
                    if(fontSetByCss == false) {
                        NodeHelper.reapplyCSS(CodeInputControl.this);
                    }
                }

                @Override
                public CssMetaData<CodeInputControl,Font> getCssMetaData() {
    ...
    public void paste() {
        ...
        // Sometimes clipboard have string but hasString() returns false
    ...
    private boolean filterAndSet(String value) {
        ...
        if (formatter != null && formatter.getFilter() != null && !text.isBound()) {
            TextFormatter.Change change = new TextFormatter.Change(
                    CodeInputControl.this, getFormatterAccessor(), 0, length, value, 0, 0);
            change = formatter.getFilter().apply(change);
            if (change == null) {
                return false;
            }
    ...
    private static class StyleableProperties {
        private static final FontCssMetaData<CodeInputControl> FONT =
                new FontCssMetaData<>("-fx-font", Font.getDefault()) {

                    @Override
                    public boolean isSettable(CodeInputControl n) {
                        return n.font == null || !n.font.isBound();
                    }

                    @Override
                    public StyleableProperty<Font> getStyleableProperty(CodeInputControl n) {
    ...
    private class TextInputControlFromatterAccessor implements FormatterAccessor {
        ...
        public int getTextLength() {
            return CodeInputControl.this.getLength();
        }

        @Override
        public String getText(int begin, int end) {
            return CodeInputControl.this.getText(begin, end);
        }

        @Override
        public int getCaret() {
            return CodeInputControl.this.getCaretPosition();
        }

        @Override
        public int getAnchor() {
            return CodeInputControl.this.getAnchor();
        }
    }
